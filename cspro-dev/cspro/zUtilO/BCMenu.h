...
class CLASS_DECL_ZUTILO BCMenuData
{
...
class CLASS_DECL_ZUTILO BCMenu : public CMenu
{
...
public:
    ...
    BOOL AppendMenuW(UINT nFlags,UINT nIDNewItem=0,const wchar_t *lpszNewItem=NULL,int nIconNormal=-1);
    BOOL AppendMenuW(UINT nFlags,UINT nIDNewItem,const wchar_t *lpszNewItem,CImageList *il,int xoffset);
    BOOL AppendMenuW(UINT nFlags,UINT nIDNewItem,const wchar_t *lpszNewItem,CBitmap *bmp);
    BOOL AppendODMenuA(LPCSTR lpstrText,UINT nFlags = MF_OWNERDRAW,UINT nID = 0,int nIconNormal = -1);
    BOOL AppendODMenuW(const wchar_t *lpstrText,UINT nFlags = MF_OWNERDRAW,UINT nID = 0,int nIconNormal = -1);
    BOOL AppendODMenuA(LPCSTR lpstrText,UINT nFlags,UINT nID,CImageList *il,int xoffset);
    BOOL AppendODMenuW(const wchar_t *lpstrText,UINT nFlags,UINT nID,CImageList *il,int xoffset);
...
// CSPro additions
public:
    // the menu object is passed as a templated type because the AppendMenu method is not defined as a virtual method
    template<typename MT>
    static void AppendMenuItems(MT& menu, bool enabled, std::initializer_list<std::tuple<UINT, const TCHAR*>> items)
    {
        UINT flags = MF_STRING | ( enabled ? 0 : MF_GRAYED );

        for( const auto& [id, text] : items )
            menu.AppendMenu(flags, id, text);
    }

    template<typename MT>
    static void AppendMenuItems(MT& menu, std::initializer_list<std::tuple<UINT, const TCHAR*>> items)
    {
        AppendMenuItems(menu, true, items);
    }

    void AppendMenuItems(bool enabled, std::initializer_list<std::tuple<UINT, const TCHAR*>> items)
    {
        AppendMenuItems(*this, enabled, items);
    }

    void AppendMenuItems(std::initializer_list<std::tuple<UINT, const TCHAR*>> items)
    {
        AppendMenuItems(*this, items);
    }
};
