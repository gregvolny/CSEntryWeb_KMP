...
void CFlatSplitterWnd::RecalcLayout()
{
    ...
    if (m_bHasHScroll)
    {
        ...
        for (int col = 0; col < m_nCols; col++)
        {
            ...
            ASSERT(pScrollBar != NULL);
            int cxc = m_pColInfo[col].nCurSize;
            if (col == 0 && m_nCols < m_nMaxCols)
                x += cxSplitterBox, cxc -= cxSplitterBox;
            DeferClientPos(&layout, pScrollBar, x, y, cxc, cy, TRUE);
            x += cxc + m_cxSplitterGap;
        }
    }
    ...
    if (m_bHasVScroll)
    {
        ...
        for (int row = 0; row < m_nRows; row++)
        {
            ...
            ASSERT(pScrollBar != NULL);
            int cyr = m_pRowInfo[row].nCurSize;
            if (row == 0 && m_nRows < m_nMaxRows)
                y += cySplitterBox, cyr -= cySplitterBox;
            DeferClientPos(&layout, pScrollBar, x, y, cx, cyr, TRUE);
            y += cyr + m_cySplitterGap;
        }
    }
    ...
    //BLOCK: Reposition all the panes
    {
        ...
        for (int col = 0; col < m_nCols; col++)
        {
            int cxp = m_pColInfo[col].nCurSize;
            int y = rectClient.top;
            for (int row = 0; row < m_nRows; row++)
            {
                int cyp = m_pRowInfo[row].nCurSize;
                CWnd* pWnd = GetPane(row, col);
                DeferClientPos(&layout, pWnd, x, y, cxp, cyp, FALSE);
                y += cyp + m_cySplitterGap;
            }
            x += cxp + m_cxSplitterGap;
        }
    }
