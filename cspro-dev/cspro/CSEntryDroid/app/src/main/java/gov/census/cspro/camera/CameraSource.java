 ...
 * preview or detector results are weak, then you may want to consider increasing FPS.
 * <p/>
...
public class CameraSource {
    ...
    private double mRequestedPreviewWidth = 1024;
    private double mRequestedPreviewHeight = 768;


    private String mFocusMode = null;
    private String mFlashMode = null;

    // This instances need to be held onto to avoid GC of their underlying resources.  Even though
    ...
    public static class Builder {
         ...
         */
        protected Builder(Context context, Detector<?> detector) {
            if (context == null) {
         ...
         */
        Builder setRequestedPreviewSize(double width, double height) {
    ...
    protected CameraSource start() throws IOException {
        synchronized (mCameraLock) {
            if (mCamera != null) {
                return this;
            }
     ...
     */
    @RequiresPermission(Manifest.permission.CAMERA)
    protected CameraSource start(SurfaceHolder surfaceHolder) throws IOException {
        synchronized (mCameraLock) {
            if (mCamera != null) {
                return this;
            }
    ...
    private static SizePair selectSizePairByAspectRatio(double ratio, List<SizePair> validPreviewSizes){
        SizePair selectedPair = null;

        double minDiff = Double.MAX_VALUE;
        for (SizePair sizePair : validPreviewSizes) {
            Size size = sizePair.previewSize();
            double diff = Math.abs(((double)size.getWidth()) / ((double)size.getHeight()) - ratio);
            if (diff < minDiff) {
     ...
     */
    private static SizePair selectSizePair(Camera camera, double desiredWidth, double desiredHeight) {
        List<SizePair> validPreviewSizes = generateValidPreviewSizeList(camera);

        //Handling custom aspect ratio here
        if ((int)desiredHeight == 1) {
            return selectSizePairByAspectRatio(desiredWidth, validPreviewSizes);
        }
        ...
        for (SizePair sizePair : validPreviewSizes) {
            Size size = sizePair.previewSize();
            int diff = Math.abs(size.getWidth() - (int) desiredWidth) +
                Math.abs(size.getHeight() - (int) desiredHeight);
            if (diff < minDiff) {
                selectedPair = sizePair;
                minDiff = diff;
            }
        ...
        public void run() {
            ...
            while (true) {
                ...
                try {
                    ...
                    //Timber.e(t, "Exception thrown from  barcode Detector's receiver.");
                } finally {
