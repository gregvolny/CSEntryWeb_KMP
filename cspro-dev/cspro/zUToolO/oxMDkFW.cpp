...
{
    ContainedBarType Type = GetContainedBarType();
    if (Type == MRCSizeBar)
    {
...
int COXSizableMiniDockFrameWnd::OnMouseActivate(CWnd* pDesktopWnd, UINT nHitTest, UINT message)
{
    ...
    if(pBar->IsKindOf(RUNTIME_CLASS(COXSizeViewBar)))
    {
        pBar->SendMessage(UWM::UTool::ACTIVATEVIEWBAR,TRUE,ID_SOURCE_MINIDOCK);
    }
...
COXSizableMiniDockFrameWnd::ContainedBarType COXSizableMiniDockFrameWnd::GetContainedBarType()
    {
...
void COXSizableMiniDockFrameWnd::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
    ...
    if(pBar!=NULL)
    {
        ...
        if(pBar->IsKindOf(RUNTIME_CLASS(COXSizeViewBar)))
        {
            pBar->SendMessage(UWM::UTool::ACTIVATEVIEWBAR,nState==WA_INACTIVE ? FALSE : TRUE,
                ID_SOURCE_MINIDOCK);
        }
    }
}
...
//SAVY added this function to make it compile in vc 7 MFC function obsolete
//The function seems to be of not much importance .get bak to this later .
//SAVY &&& 05/03/2004
void COXSizableMiniDockFrameWnd::InvertSysMenu()
{
