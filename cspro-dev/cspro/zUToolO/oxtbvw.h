...
//SAVY 11/29/1999 cos of redefinition problem
...
typedef struct _tagPAGEINFO
{
    ...
    void GetScrollInfo(CWnd* pThisWnd, BOOL bHorzScrlBar)
    {
        ASSERT_VALID(pThisWnd);

        if(bHorzScrlBar)
        {
            scrlInfoHorz.cbSize=sizeof(SCROLLINFO);
            pThisWnd->GetScrollInfo(SB_HORZ,&scrlInfoHorz);
            if(bHasScrollVert)
            {
...
//              scrlInfoHorz.nPage-=::GetSystemMetrics(SM_CXVSCROLL);
        ...
        else
        {
            ...
            pThisWnd->GetScrollInfo(SB_VERT,&scrlInfoVert);
            if(bHasScrollHorz)
            {
                scrlInfoVert.nMax-=1;
...
public:
    ...
    typedef enum _tagHITTEST
    {
        ...
        TABBTNAREA=-9,
        SIZEBAR=-10,
        TAB_NONE=-11 //SAVY Changed this from NONE to TAB_NONE cos of name clash 11/29/1999
    } HITTEST;
...
public:
    ...
    virtual void UpdateScrollState();
...
//OX_API_DECL //SAVY Replaced this 11/29/1999 for export
OX_CLASS_DECL COXTabViewContainer* PASCAL
