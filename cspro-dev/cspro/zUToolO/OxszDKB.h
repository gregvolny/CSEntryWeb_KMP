...
typedef struct _ROWSIZEINFO
{
    ...
    int nMinHeight;                 // minimum height that might be set to this row
    int nMinWidth;                  // minimum width that might be set to this row
} ROWSIZEINFO;
...
// Miscellaneous
    ...
    {
        return pWnd->SetWindowPos(NULL,0,0,size.cx,size.cy,SWP_NOMOVE|SWP_NOZORDER);
    }

    void ResizeBar(COXSizeControlBar* pBar, BOOL bMaximize);
...
protected:
    ...
    // Splitter rectangles...
    inline COXSplitterRect* GetSplitter(int i)
    {
