...
#include "StdAfx.h"
//#include <..\src\afximpl.h> // for AUX_DATA struct
            ...
            if (pBar!=NULL && ::IsWindow(pBar->GetSafeHwnd()) && pBar->IsVisible())
            {
                ...
                if (bHorz)
                {
                    ...
                    if (!bWrapped)
                    {
                        ...
                        nWidth = (int) std::max((LONG) nWidth, sizeBar.cy);
                    }
                }
                else
                {
                    ...
                    if (!bWrapped)
                    {
                        ...
                        nWidth = (int) std::max((LONG) nWidth, sizeBar.cx);
                    }
                }
...
void COXSizeDockBar::OnPaint()
{
    ...
    for(int nIndex=m_SplitArr.GetUpperBound(); nIndex>=0; nIndex--)
    {
