...
// CSPRO CHANGES: to allow paths to have non-ANSI characters, some
// changes have been made, marked with HTTPLIB_CSPRO
...
struct Response {
  ...
  void set_content(const char *s, size_t n); // HTTPLIB_CSPRO added
...
inline void read_file(const std::string &path, std::string &out) {
  ...
  if( file != nullptr )
  {
    ...
    {
      out.resize(static_cast<size_t>(size));
      fread(&out[0], 1, static_cast<size_t>(size), file);
    }
...
inline std::string to_lower(const char *beg, const char *end) { // HTTPLIB_CSPRO moved up in the file to make accessible to find_content_type
  std::string out;
  auto it = beg;
  while (it != end) {
...
find_content_type(const std::string &path,
  ...
  ext = to_lower(ext.data(), ext.data() + ext.length()); // HTTPLIB_CSPRO made the extension lowercase
...
inline void Response::set_content(const char *s, size_t n) { // HTTPLIB_CSPRO added...
  body.assign(s, n);
