...
//
//  Description:
//       Header for Data Dictionary dictionary properties grid application
//
...
class CDictGrid : public CDDGrid
{
public:
    CDictGrid();
    ...
    bool IsEditingNameOrLabelAndCanUndo() const;

    static constexpr long GetFirstLevelRow() { return 3; }
...
protected:
    ...
    //***** Over-ridable Notify Functions (derived from CUGCtrl)
...
private:
    std::vector<size_t> GetSelectedLevels();

    void EditCopy(bool bCut);
    void EditDelete(const std::vector<size_t>& selected_levels);
