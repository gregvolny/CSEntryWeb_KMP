...
void CDELevel::serialize(Serializer& ar)
{
    ...
    printf("[CDELevel] serialize starting, calling CDEFormBase::serialize...\n");
    fflush(stdout);
#endif
    CDEFormBase::serialize(ar);

#ifdef __EMSCRIPTEN__
    printf("[CDELevel] CDEFormBase done, reading level data...\n");
    fflush(stdout);
#endif
...
#ifdef __EMSCRIPTEN__
    printf("[CDELevel] Level data read, numGroups=%d\n", iGetNumGroups);
    fflush(stdout);
#endif
    ...
    {
#ifdef __EMSCRIPTEN__
        printf("[CDELevel] Processing group %d/%d...\n", i, iGetNumGroups);
        fflush(stdout);
#endif
        if( ar.IsSaving() )
        {
...
#ifdef __EMSCRIPTEN__
    printf("[CDELevel] serialize done\n");
    fflush(stdout);
#endif
}
