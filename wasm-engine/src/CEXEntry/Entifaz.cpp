...
DEFLD* CEntryIFaz::C_EndGroup( bool bPostProc ) {
    ...
    // [VSCode Extension] Always allow EndGroup to proceed. 
    // We rely on CsDriver and logic validation to determine if leaving the group is permitted.
    // In System Controlled mode (PathOn), this allows "End Group" (e.g., F12/Ctrl+/) to work for Rosters/Groups.
    bool    bCanUseEnd  = true; // Was: m_pEngineSettings->IsPathOff();
