...
// downloaded from: http://www.codeproject.com/Articles/1146/CString-clone-Using-Standard-C
// last update: 7 Dec 2011
// look for some comments prefaced with CSPro
...
// MACRO: SS_MINIMAL_WIN
// ---------------------
//      Windows universal apps don't have access to all the APIs that
//      regular WIN32 desktop apps do such as LoadString.
#if !defined(SS_ANSI) && defined(WIN32)
    #if defined(WINAPI_FAMILY_PARTITION)
        #if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP)
            #define SS_MINIMAL_WIN
        #endif
...
#define	NOMINMAX // Windows.h defines min/max macros by default, disable that
#include <WTYPES.H>
...
#if defined (_MSC_VER) && (_MSC_VER >= 1100) && !defined(SS_MINIMAL_WIN)
#include <comdef.h>
...
// we can use the version defined above
...
// CSPro 20131207 switched out the Format functions to use a new sprintf algorithm, since the
// standard ones didn't work with unicode characters in the crystax build
...
{
	return ap_vsnprintf(pW, nCount, pFmtW, vl);
}
...
// -----------------------------------------------------------------------------
#if defined (SS_WIN32) && !defined(SS_ANSI) && !defined(SS_MINIMAL_WIN)
...
#define SSRES(pctstr)  LOWORD(reinterpret_cast<unsigned long>(pctstr))
    ...
    CStdStr(const MYTYPE& str) : m_str(SSREF(str.m_str))
    {}

    CStdStr(const std::string& str)
    {
        ssasn(m_str, SSREF(str));
    }
    ...
    CStdStr(PCMYSTR pT, MYSIZE n) : m_str(NULL == pT ? MYTYPE().m_str.c_str() : pT, n)
    {}

    explicit CStdStr(CT c, int nRepeat = 1) : m_str(nRepeat, c)
    {}
    ...
    */
    // CSPro 20131226 it's unclear why they reversed the nSize/ch arguments above (compared to the MFC implementation)
    CStdStr(MYVAL ch, MYSIZE nSize, const MYALLOC& al = MYALLOC())
        : m_str(nSize, ch, al)
    {}
    ...
    {
        if (bstr.length() > 0)
            m_str.append(static_cast<PCMYSTR>(bstr), bstr.length());
    }
#endif
    ...
    {
        ssasn(m_str, reinterpret_cast<PCSTR>(pU));
        return *this;
    }
#endif
    ...
    {
        if (bstr.length() > 0) {
            m_str.assign(static_cast<PCMYSTR>(bstr), bstr.length());
            return *this;
        } else {
            m_str.erase();
            return *this;
...
#if defined ( _MSC_VER ) && ( _MSC_VER < 1200 )
        if (!m_str.empty() &&
            (pC < m_str.data() || pC > m_str.data() + m_str.capacity())) {
            m_str.erase();
        }
#endif
        Q172398(m_str);
    ...
    {
        return m_str.assign(pT, MYBASE::traits_type::length(pT));
    }
    ...
    MYTYPE& assign(MYCITER iterFirst, MYCITER iterLast)
    {
        ...
        if (iterFirst < m_str.begin() ||
            iterFirst > m_str.begin() + m_str.size()) {
            m_str.erase()
        }
#endif
        m_str.replace(m_str.begin(), m_str.end(), iterFirst, iterLast);
        return *this;
    }
#endif
    ...
    MYTYPE& operator+=(const _bstr_t& bstr)
    {
        return m_str.operator+=(static_cast<PCMYSTR>(bstr));
    }
#endif
    ...
    MYTYPE& ToUpper(const std::locale& loc = std::locale())
    {
        ...
        std::transform(m_str.begin(),
            ...
            SSToUpper<CT>());
#else
            [&loc](const CT& t) {return SSToUpper<CT>()(t,loc);});
#endif
        ...
        //      if ( !empty() )
        //      {
        //          ssupr(m_str.GetBuf(), m_str.size(), loc);
        //          m_str.RelBuf();
        //      }
    ...
    MYTYPE& ToLower(const std::locale& loc = std::locale())
    {
        ...
        std::transform(m_str.begin(),
            ...
            SSToLower<CT>());
#else
            [&loc](const CT& t) {return SSToLower<CT>()(t,loc);});
#endif
        ...
        //      if ( !empty() )
        //      {
        //          sslwr(m_str.GetBuf(), m_str.size(), loc);
        //          m_str.RelBuf();
        //      }
        return *this;
    }
    ...
    {
        if (static_cast<int>(m_str.size()) < nMinLen)
            m_str.resize(static_cast<MYSIZE>(nMinLen));

        return m_str.empty() ? const_cast<CT*>(m_str.data()) : &(m_str.at(0));
    }

    CT* SetBuf(int nLen)
    {
        nLen = (nLen > 0 ? nLen : 0);
        if (m_str.capacity() < 1 && nLen == 0)
            m_str.resize(1);

        m_str.resize(static_cast<MYSIZE>(nLen));
        return const_cast<CT*>(m_str.data());
    }
    void RelBuf(int nNewLen = -1)
    {
        m_str.resize(static_cast<MYSIZE>(nNewLen > -1 ? nNewLen :
            sslen(m_str.c_str())));
    }
    ...
    {
        return  0 == (bUseCase ? m_str.compare(pT) : ssicmp(m_str.c_str(), pT));
    }
...
#if !defined(SS_ANSI) && !defined(SS_MINIMAL_WIN)
    ...
    void Append(const CT* szMessage)
    {
        // AppendFormat((const CT*)L"%s",szMessage);

        // CSPro 20220207 with STD_BUF_SIZE set to 1024, executing the above
        // code resulted in functions like concat failing when passed strings
        // that were 1024+ characters long ... it's not clear why AppendFormat
        // was used instead of simply appending the text to the string
        m_str.append(szMessage);
    }

    void Append(const CT* pszSrc, int nLength)
    {
        // CSPro 20220708 this method was missing
        m_str.append(pszSrc, nLength);
    }

    void AppendChar(CT ch)
    {
        m_str.append(1, ch);
    }
...
#define STD_BUF_SIZE        1024
    ...
    // -------------------------------------------------------------------------
#if defined(SS_WIN32) && !defined(SS_MINIMAL_WIN)
    ...
    {
        return sscoll(m_str.c_str(), m_str.length(), szThat, sslen(szThat));
    }

    int CollateNoCase(PCMYSTR szThat) const
    {
        return ssicoll(m_str.c_str(), m_str.length(), szThat, sslen(szThat));
    }
#endif
    int Compare(PCMYSTR szThat) const
    {
        return m_str.compare(szThat);
    }

    int CompareNoCase(PCMYSTR szThat)   const
    {
        return ssicmp(m_str.c_str(), szThat);
    }
    ...
    int Delete(int nIdx, int nCount = 1)
    {
        ...
        if (nIdx < this->GetLength())
            m_str.erase(static_cast<MYSIZE>(nIdx), static_cast<MYSIZE>(nCount));
    ...
    int Find(CT ch, int nStart) const
    {
        ...
        MYSIZE nIdx = m_str.find_first_of(ch, static_cast<MYSIZE>(nStart));
        return static_cast<int>(MYBASE::npos == nIdx ? -1 : nIdx);
    }
    ...
    int Find(PCMYSTR szSub, int nStart) const
    {
        ...
        MYSIZE nIdx = m_str.find(szSub, static_cast<MYSIZE>(nStart));
        return static_cast<int>(MYBASE::npos == nIdx ? -1 : nIdx);
    }

    int FindOneOf(PCMYSTR szCharSet) const
    {
        MYSIZE nIdx = m_str.find_first_of(szCharSet);
        return static_cast<int>(MYBASE::npos == nIdx ? -1 : nIdx);
    }
    ...
    {
        return m_str.at(static_cast<MYSIZE>(nIdx));
    }
    ...
    {
        if (static_cast<MYSIZE>(nIdx) > m_str.size() - 1)
            m_str.append(1, ch);
        else
            m_str.insert(static_cast<MYSIZE>(nIdx), 1, ch);

        return GetLength();
    }
    int Insert(int nIdx, PCMYSTR sz)
    {
        if (static_cast<MYSIZE>(nIdx) >= m_str.size())
            m_str.append(sz, static_cast<MYSIZE>(sslen(sz)));
        else
            m_str.insert(static_cast<MYSIZE>(nIdx), sz);
    ...
    MYTYPE Left(int nCount) const
    {
        ...
        nCount = SSMAX(0, SSMIN(nCount, static_cast<int>(m_str.size())));
        return m_str.substr(0, static_cast<MYSIZE>(nCount));
    }
    ...
    MYTYPE Mid(int nFirst, int nCount) const
    {
        ...
        int nSize = static_cast<int>(m_str.size());
        ...
        return m_str.substr(static_cast<MYSIZE>(nFirst),
    ...
    int Remove(CT ch)
    {
        MYSIZE nIdx = 0;
        int nRemoved = 0;
        while ((nIdx = m_str.find_first_of(ch)) != MYBASE::npos) {
            m_str.erase(nIdx, 1);
            nRemoved++;
        }
        return nRemoved;
    ...
    int Replace(CT chOld, CT chNew)
    {
        ...
        for (MYITER iter = m_str.begin(); iter != m_str.end(); iter++) {
            if (*iter == chOld) {
                *iter = chNew;
                nReplaced++;
            }
    ...
    int Replace(PCMYSTR szOld, PCMYSTR szNew)
    {
        ...
        if (0 != nOldLen) {
            ...
            if (nNewLen > nOldLen) {
                int nFound = 0;
                while (nIdx < m_str.length() &&
                    (nIdx = m_str.find(szOld, nIdx)) != MYBASE::npos) {
                    nFound++;
                    nIdx += nOldLen;
                }
                m_str.reserve(m_str.size() + nFound * (nNewLen - nOldLen));
            }
            ...
            while (nIdx < m_str.length() &&
                (nIdx = m_str.find(szOld, nIdx)) != MYBASE::npos) {
                m_str.replace(m_str.begin() + nIdx, m_str.begin() + nIdx + nOldLen,
                    szRealNew);
    ...
    {
        MYSIZE nIdx = m_str.find_last_of(ch);
        return static_cast<int>(MYBASE::npos == nIdx ? -1 : nIdx);
    }

    MYTYPE Tokenize(PCMYSTR pszTokens, int& iStart) const
    {
        if (iStart < 0)
            return MYTYPE();
        ...
        if ((pszTokens == NULL) || (*pszTokens == 0)) {
            if (iStart < GetLength()) {
                return(MYTYPE(GetString() + iStart));
            }
        } else {
            if (iStart < GetLength()) {
                MYSIZE pos = m_str.find_first_not_of(pszTokens, iStart);
                int nextNonDelimiter = (pos == MYBASE::npos) ? GetLength() : (int) pos;

                if (nextNonDelimiter < GetLength()) {
                    pos = m_str.find_first_of(pszTokens, nextNonDelimiter);
                    int nextTokenStart = (pos == MYBASE::npos) ? GetLength() : (int) pos;
                    iStart = nextTokenStart + 1;
                    return(Mid(nextNonDelimiter, nextTokenStart - nextNonDelimiter));
                }
            }
        }
    ...
    // CSPro 20121121 removing this function so that our CString-compliant classes don't accidentally compile correctly if someone writes something like sFileN.ReverseFind(_T("\\"));
    /*int ReverseFind(PCMYSTR szFind, MYSIZE pos=MYBASE::npos) const
    {
    MYSIZE nIdx = m_str.rfind(0 == szFind ? MYTYPE() : szFind, pos);
    return static_cast<int>(MYBASE::npos == nIdx ? -1 : nIdx);
    }*/
    ...
    MYTYPE Right(int nCount) const
    {
        ...
        nCount = SSMAX(0, SSMIN(nCount, static_cast<int>(m_str.size())));
        return m_str.substr(m_str.size() - static_cast<MYSIZE>(nCount));
    }

    void SetAt(int nIndex, CT ch)
    {
        ASSERT(m_str.size() > static_cast<MYSIZE>(nIndex));
        m_str.at(static_cast<MYSIZE>(nIndex)) = ch;
    }
    ...
    {
        MYSIZE pos = m_str.find_first_of(szCharSet);
        return pos == MYBASE::npos ? *this : Left(pos);
    }

    MYTYPE SpanIncluding(PCMYSTR szCharSet) const
    {
        MYSIZE pos = m_str.find_first_not_of(szCharSet);
        return pos == MYBASE::npos ? *this : Left(pos);
    }
    ...
    void AnsiToOem()
    {
        if (sizeof(CT) == sizeof(char) && !empty()) {
            ::CharToOem(reinterpret_cast<PCSTR>(m_str.c_str()),
    ...
    {
        if (sizeof(CT) == sizeof(char) && !empty()) {
            ::OemToChar(reinterpret_cast<PCSTR>(m_str.c_str()),
                reinterpret_cast<PSTR>(GetBuf()));
        } else {
            ASSERT(false);
        }
    ...
    MYTYPE& TrimLeft()
    {
        m_str.erase(m_str.begin(),
            std::find_if(m_str.begin(), m_str.end(), NotSpace<CT>()));
    ...
    {
        m_str.erase(0, m_str.find_first_not_of(tTrim));
        return *this;
    }

    MYTYPE&  TrimLeft(PCMYSTR szTrimChars)
    {
        m_str.erase(0, m_str.find_first_not_of(szTrimChars));
        return *this;
    }
    ...
    MYTYPE& TrimRight()
    {
        ...
        MYRITER it = std::find_if(m_str.rbegin(), m_str.rend(), NotSpace<CT>());
        if (!(m_str.rend() == it))
            m_str.erase(m_str.rend() - it);

        m_str.erase(!(it == m_str.rend()) ? m_str.find_last_of(*it) + 1 : 0);
        return *this;
    }

    MYTYPE&  TrimRight(CT tTrim)
    {
        MYSIZE nIdx = m_str.find_last_not_of(tTrim);
        m_str.erase(MYBASE::npos == nIdx ? 0 : ++nIdx);
        return *this;
    }

    MYTYPE&  TrimRight(PCMYSTR szTrimChars)
    {
        MYSIZE nIdx = m_str.find_last_not_of(szTrimChars);
        m_str.erase(MYBASE::npos == nIdx ? 0 : ++nIdx);
        return *this;
    }
    ...
    void            FreeExtra()
    {
        ...
        m_str.swap(mt.m_str);
        if (!mt.m_str.empty())
            m_str.assign(mt.m_str.c_str(), mt.m_str.size());
    }
    ...
    {
        return m_str.operator[](static_cast<MYSIZE>(nIdx));
    }
...
#define SSSO_COMPRESS   0x02    // the string is compressed
    ...
    ULONG StreamSize() const
    {
        ...
        ASSERT(m_str.size() * sizeof(CT) < 0xffffffffUL - sizeof(SSSHDR));
        return (m_str.size() * sizeof(CT)) + sizeof(SSSHDR);
    }
    ...
    HRESULT StreamSave(IStream* pStream) const
    {
        ...
        if (FAILED(hr = pStream->Write(&hdr, sizeof(SSSHDR), 0))) {
            ...
            ;       // nothing to write
        } else if (FAILED(hr = pStream->Write(m_str.c_str(),
            m_str.size() * sizeof(CT), 0))) {
    ...
    friend CStdStr<char> operator+(const CStdStr<char>& s1, const CStdStr<char>& s2);
    friend CStdStr<char> operator+(const CStdStr<char>& s1, char t);
    friend CStdStr<char> operator+(char t, const CStdStr<char>& s1);
    friend CStdStr<char> operator+(const CStdStr<char>& s1, PCSTR pA);
    friend CStdStr<char> operator+(PCSTR pA, const CStdStr<char>& sA);
    friend CStdStr<char> operator+(const CStdStr<char>& s1, const CStdStr<wchar_t>& s2);
    friend CStdStr<wchar_t> operator+(const CStdStr<wchar_t>& s1, const CStdStr<wchar_t>& s2);
    friend CStdStr<char> operator+(const CStdStr<char>& s1, PCWSTR pW);
#ifdef UNICODE
    friend CStdStr<wchar_t> operator+(PCWSTR pW, const CStdStr<char>& sA);
    friend CStdStr<wchar_t> operator+(PCSTR pA, const CStdStr<wchar_t>& sW);
#else
    friend CStdStr<char> operator+(PCWSTR pW, const CStdStr<char>& sA);
    friend CStdStr<char> operator+(PCSTR pA, const CStdStr<wchar_t>& sW);
#endif

    friend CStdStr<wchar_t> operator+(const CStdStr<wchar_t>& s1, wchar_t t);
    friend CStdStr<wchar_t> operator+(wchar_t t, const CStdStr<wchar_t>& s1);
    friend CStdStr<wchar_t> operator+(const CStdStr<wchar_t>& s1, PCWSTR pW);
    friend CStdStr<wchar_t> operator+(PCWSTR pW, const CStdStr<wchar_t>& sW);
    friend CStdStr<wchar_t> operator+(const CStdStr<wchar_t>& s1, const CStdStr<char>& s2);
    friend CStdStr<wchar_t> operator+(const CStdStr<wchar_t>& s1, PCSTR pA);
...
// Actually it is necessary to export these from the DLL since
// in zUtilO IMSAString derives from this class and is exported
// which forces this class to be implicitly exported by zUtilO
// too. This leads to multiply defined symbols in compilation
// units that use this class without including IMSAString
// since they create a local copy of the inlined methods
// which conflict with the same methods exported from
// zUtilO.dll at link time.

// On a related note, this class originally derived from
// std::basic_string<> which, in addition to just being
// against the golden rule "prefer composition to inheritance",
// forces std::basic_string<wchar_t> and std:basic_string<char>
// to be implicitly DLL exported too. That leads to some link
...
#else
#define SSDLLSPEC __declspec(dllimport)
#endif
template class SSDLLSPEC CStdStr<char>;
template class SSDLLSPEC CStdStr<wchar_t>;
#endif
...
inline CStdStringA operator+(const CStdStringA& s1, const CStdStringA& s2)
{
    ...
    sRet.m_str.append(s2.m_str);
    return sRet;
}
inline CStdStringA operator+(const CStdStringA& s1, char t)
{
    CStdStringA sRet(SSREF(s1));
    sRet.m_str.append(1, t);
    return sRet;
}
inline CStdStringA operator+(char t, const CStdStringA& s1)
{
    CStdStringA sRet(t);
    sRet.m_str.append(s1);
    return sRet;
}
inline CStdStringA operator+(const CStdStringA& s1, PCSTR pA)
{
    CStdStringA sRet(SSREF(s1));
    sRet.m_str.append(pA);
    return sRet;
}
inline CStdStringA operator+(PCSTR pA, const CStdStringA& sA)
{
    CStdStringA sRet;
    CStdStringA::MYSIZE nObjSize = sA.m_str.size();
    CStdStringA::MYSIZE nLitSize =
        static_cast<CStdStringA::MYSIZE>(sslen(pA));

    sRet.m_str.reserve(nLitSize + nObjSize);
    sRet.m_str.assign(pA);
    sRet.m_str.append(sA);
    return sRet;
}
...
inline CStdStringW operator+(const CStdStringW& s1, wchar_t t)
{
    CStdStringW sRet(SSREF(s1));
    sRet.m_str.append(1, t);
    return sRet;
}
inline CStdStringW operator+(wchar_t t, const CStdStringW& s1)
{
    CStdStringW sRet(t);
    sRet.m_str.append(s1);
    return sRet;
...
{
    CStdStringW sRet(SSREF(s1));
    sRet.m_str.append(pW);
    return sRet;
}
inline CStdStringW operator+(PCWSTR pW, const CStdStringW& sW)
{
    CStdStringW sRet;
    CStdStringW::MYSIZE nObjSize = sW.m_str.size();
    CStdStringA::MYSIZE nLitSize =
        static_cast<CStdStringW::MYSIZE>(sslen(pW));

    sRet.m_str.reserve(nLitSize + nObjSize);
    sRet.m_str.assign(pW);
    sRet.m_str.append(sW);
    return sRet;
}
...
inline bool operator==(
    const CStdStr<CT>& left,
    const CStdStr<CT>& right)
{
...
struct FmtArg<CStdStringA>
{
    ...
    PCSTR operator()() const { return a_.GetString(); }
...
struct FmtArg<CStdStringW>
{
    ...
    PCWSTR operator()() const { return a_.GetString(); }
...
// REMARKS:
//      These structs may be used in Standard C++ Library
...
struct StdStringLessNoCaseW
{
    ...
    {
        return ssicmp(sLeft.GetString(), sRight.GetString()) < 0;
    }
};
