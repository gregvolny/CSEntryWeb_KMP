...
CoreEntryPage* CoreEntryEngineInterface::ProcessFieldPostMovement(CDEField* pField)
{
    ...
    if( pField != nullptr )
    {
        // NOTE (Web/WASM): DisplayTogether blocks render multiple fields on one page.
        // The web UI needs the engine current field to be the specific field reached by
        // navigation (e.g., moving from FIRST_NAME to LAST_NAME). Forcing a reenter to
        // the first field prevents advancing out of the block because the engine never
        // sees the last field as current.
...
{
    // Safety check: if no current page, just try to end the group anyway
    if( m_currentEntryPage == nullptr )
    {
        CDEField* pField = (CDEField*)m_pRunAplEntry->EndGroup(false);
        return ProcessFieldPostMovement(pField);
    }
    
    const std::vector<CoreEntryPageField>& pageFields = m_currentEntryPage->GetPageFields();
    
    for( size_t i = 0; i < pageFields.size(); i++ )
    {
        const CoreEntryPageField& page_field = pageFields[i];
        CDEField* field = page_field.GetField();
        
        if( field == nullptr )
            continue;
        
        const CDictItem* dictItem = field->GetDictItem();
        
        if( dictItem == nullptr )
            continue;
        
        const CDictRecord* record = dictItem->GetRecord();
        
        if( record == nullptr )
            continue;
        
        // don't endgroup if the field is an ID item
        if( record->GetSonNumber() == COMMON )
        {
            return m_currentEntryPage;
        }
    }

    // Always run postproc when trying to leave a group, to ensure validation logic runs
    // (e.g. required fields, error messages).
    // The case of "Empty Row in Roster" is handled specifically below by checking bInAddMode and bOccurrenceHasData.
    bool bRunPostProc = true;
    ...
    CDEGroup* pParentGroup = pCurrentField->GetParent();
    bool bInAddMode = m_pRunAplEntry->InAddMode();

    if( bInAddMode )
    {
        int maxOccs = pParentGroup->GetMaxLoopOccs();

        if( maxOccs > 1 )
        {
            int iCurOccurrence = pParentGroup->GetCurOccurrence();
            bool bOccurrenceHasData = false;
            int numItems = pParentGroup->GetNumItems();

            for( int i = 0; !bOccurrenceHasData && i < numItems; i++ )
            {
                CDEItemBase* pItem = pParentGroup->GetItem(i);
                
                // Only process fields - skip groups, text, etc.
                if (pItem->GetFormItemType() != CDEFormBase::Field)
                {
                    // printf("[CoreEntryEngineInterface::EndGroup] Skipping non-field item at index %d\n", i);
                    continue;
                }

                CDEField* pRosterField = (CDEField*)pItem;
    ...
    // [VSCode Extension] Synchronize engine occurrence state before EndGroup.
    // In System Controlled mode, leaving a roster row via EndGroup might trigger PostProc 
    // while the internal engine (CsDriver) still has a stale or zero occurrence index for the group,
    // leading to "Invalid subscript (0)" errors.
    // We force the engine to recognize the current field and occurrence by explicitly moving to it.
    if( pCurrentField != nullptr && bRunPostProc )
    {
        int iOcc = pCurrentField->GetRuntimeOccurrence();
        if( iOcc > 0 )
        {
             m_pRunAplEntry->MoveToField(pCurrentField, false);
        }
    }
...
CoreEntryPage* CoreEntryEngineInterface::SetFieldValueAndAdvance(const CString& value)
{
    ...
    // 
    // DisplayTogether blocks render multiple fields on the same page. We want:
    // - Next within the block until the last field
    // - Then advance past the block (to the next question/page)
    //
    // We approximate Android/MFC behavior by only setting advance_past_block when the current
    // field is the last field on a multi-field page.
    bool advance_past_block = false;
    try
    {
        const auto& page_fields = m_currentEntryPage->GetPageFields();
        if( page_fields.size() > 1 )
        {
            CString currentFieldName = pCurrentField->GetDictItem()->GetName();
            size_t currentIndex = 0;
            for( size_t i = 0; i < page_fields.size(); ++i )
            {
                if( page_fields[i].GetName() == currentFieldName )
                {
                    currentIndex = i;
                    break;
                }
            }
            advance_past_block = ( currentIndex + 1 >= page_fields.size() );
        }
    }
    catch( ... )
    {
        // Best-effort: fall back to field-by-field navigation.
        advance_past_block = false;
    }

    CDEField* pNextField = (CDEField*)m_pRunAplEntry->NextField(TRUE, advance_past_block);
