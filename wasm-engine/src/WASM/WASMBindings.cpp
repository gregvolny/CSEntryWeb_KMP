...
public:
    ...
    val getCurrentPage()
    {
        ...
        for (size_t i = 0; i < pageFields.size(); ++i)
        {
            ...
            // Android-side JSON uses fieldNumber/occurrenceNumber; provide the same keys for KMP parsing.
            fieldObj.set("fieldNumber", field.GetSymbol());
            ...
            fieldObj.set("occurrenceNumber", indexes.getIndexValue(0));
            fieldObj.set("occurrenceNumber2", indexes.getIndexValue(1));
            fieldObj.set("occurrenceNumber3", indexes.getIndexValue(2));
    ...
    void processPossibleRequests()
    {
        ...
        try
        {
            m_engine->ProcessPossibleRequests();
        }
        catch (...)
        {
            // best-effort
    ...
    // Uses the engine's current field (from page->GetField()), not always index 0
    bool setFieldValue(const std::string& value)
    {
        ...
        // Get the CURRENT field from the engine (not always index 0!)
        CDEField* currentField = page->GetField();
        if (!currentField) {
            printf("[WASM] setFieldValue: currentField is null, falling back to index 0\n");
            // Fallback to index 0
            CoreEntryPageField& field = page->GetPageField(0);
            return setFieldValueOnPageField(field, value);
        }
        
        // Find the matching CoreEntryPageField by name
        CString currentFieldName = currentField->GetDictItem()->GetName();
        printf("[WASM] setFieldValue: currentField=%s\n", CStringToStdString(currentFieldName).c_str());
        
        for (size_t i = 0; i < pageFields.size(); ++i) {
            if (pageFields[i].GetName() == currentFieldName) {
                CoreEntryPageField& field = page->GetPageField(static_cast<int>(i));
                return setFieldValueOnPageField(field, value);
            }
        }
        
        printf("[WASM] setFieldValue: current field not found in page fields\n");
        return false;
    }
    
    // Set field value by field name - for batch updates in blocks
    bool setFieldValueByName(const std::string& fieldName, const std::string& value)
    {
        printf("[WASM] setFieldValueByName called: field=%s, value=%s\n", fieldName.c_str(), value.c_str());
        ...
        auto& pageFields = page->GetPageFields();
        CString targetName = StdStringToCString(fieldName);
        
        for (size_t i = 0; i < pageFields.size(); ++i) {
            if (pageFields[i].GetName() == targetName) {
                CoreEntryPageField& field = page->GetPageField(static_cast<int>(i));
                printf("[WASM] setFieldValueByName: found field at index %zu\n", i);
                return setFieldValueOnPageField(field, value);
            }
        }
        
        printf("[WASM] setFieldValueByName: field '%s' not found in current page\n", fieldName.c_str());
        return false;
    }
    
    // Helper function to set value on a CoreEntryPageField
    bool setFieldValueOnPageField(CoreEntryPageField& field, const std::string& value)
    {
        if (field.IsReadOnly())
        {
        ...
        try
        {
            ...
            {
                // Parse numeric value
                // For empty string, use NOTAPPL (blank/not applicable) instead of 0.0
                // This properly triggers validation when the field is blank
                double numValue = NOTAPPL;
                if (!value.empty())
                {
                    numValue = std::stod(value);
                }
                printf("[WASM] Setting numeric field value: %s -> %f (empty=%d, NOTAPPL=%f)\n", 
                       CStringToStdString(field.GetName()).c_str(), numValue, value.empty() ? 1 : 0, NOTAPPL);
            ...
            printf("[WASM] Field value set successfully: %s\n", CStringToStdString(field.GetName()).c_str());
            return true;
        }
        catch (const std::exception& e)
        {
    ...
    val endGroup()
    {
        ...
        printf("[WASM] endGroup() called\n");
        fflush(stdout);
        
        CoreEntryPage* page = m_engine->EndGroup();
        
        printf("[WASM] endGroup(): EndGroup returned page=%p\n", (void*)page);
        fflush(stdout);
        
        if (page) {
            CDEField* field = page->GetField();
            if (field) {
                printf("[WASM] endGroup(): current field=%ls\n", field->GetName().GetString());
                fflush(stdout);
            }
        }
...
EMSCRIPTEN_BINDINGS(cspro_engine)
{
    ...
    class_<CSProEngine>("CSProEngine")
        ...
        .function("processPossibleRequests", &CSProEngine::processPossibleRequests, async())
        ...
        .function("setFieldValueByName", &CSProEngine::setFieldValueByName)
