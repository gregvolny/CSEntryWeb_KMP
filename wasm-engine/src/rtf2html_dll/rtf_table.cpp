...
#include <algorithm> // JH added for compilation with VC++ on Windows
                 ...
                 ++span_row)
            {
                cell_def_2 =
                    ...
                    (*span_row)->CellDefs->end(),
                        [&cell_def](table_cell_def* tcd) {return tcd->right_equals((*cell_def)->Right); });
           ...
           cell!=(*row)->Cells.end(); ++cell, ++cell_def)
      {
         ...
         if (!(*cell_def)->Merged)
         {
            ...
            for (row2=row; row2!=span_row; ++row2)
            {
                cell_def_2 =
                    std::find_if((*row2)->CellDefs->begin(),
                                (*row2)->CellDefs->end(),
                                [left](table_cell_def* tcd) {return tcd->right_equals(left); });
               ...
               cell_def_2=
                   std::find_if((*row2)->CellDefs->begin(),
                                (*row2)->CellDefs->end(),
                                [right](table_cell_def* tcd) {return tcd->left_equals(right); });
               if (cell_def_2!=(*row2)->CellDefs->end())
               {
            ...
            if ((*cell)->Text[0]>0)
               ...
               result+="&#160;"; // JH added for xhtml compliance
            result+="</td>";
         }
      }
      result+="</tr>";
