...
// this implementation of C++20's std::span comes from: https://github.com/tcbrindle/span/blob/master/include/tcb/span.hpp
// it is followed by a version, cs::span, that allows spans to be constructed from a std::initializer_list
...
{
    template<typename ElementType>
    class span : public tcb::span<ElementType>
    {
    public:
        using tcb_span = tcb::span<ElementType>;
        using tcb_span::tcb_span;

        // construct a one-element span
        span(ElementType& element)
            :   tcb_span(&element, 1)
        {
        }

        // construct a span from an initializer_list
        span(std::initializer_list<ElementType> elements)
            :   tcb_span(elements.begin(), elements.size())
        {
        }
