...
public:
    ...
    int           m_iRows;
    int           m_iCols;
    CString       m_sPifFileName;
    int           m_iEllipsisIndex;
    CUGMaskedEdit m_pPifEdit;
    CStringArray  m_arrMultFiles;

    void SetGridData(const CArray<PIFINFO*, PIFINFO*>& gridData, int windowWidth);

    PIFINFO* GetRowInfo(long row);
    void GetDialogTitleAndFilter(PIFINFO* pPifInfo, CString& sTitle, CString& sFilter);
    void ResetGrid(void);

    void ForceButtonClick(int rowNum) { OnCellTypeNotify(0,0,rowNum,UGCT_ELLIPSISBUTTONCLICK,0); } // GHM 20110805

    void SetDefaultInputDataFilename(CString filename);
...
private:
    bool ChooseFile(PIFINFO* pPifInfo, int col, long row);

    bool UpdateDataRepositoryRow(long row, const std::vector<ConnectionString>& connection_strings,
        DataRepositoryType default_data_repository_type = DataRepositoryType::SQLite);

    const DataFileFilter* GetSelectedDataFileFilter(long row);

    bool UpdateFilteredExtensionRow(long row, const CString& filename);

private:
    CUGEllipsisType m_ellipsis;
    CFont           m_font;

    std::map<long, const DataFileFilterManager*> m_dataFileFilterManagers;
    std::map<long, std::shared_ptr<FilteredExtensionProcessor>> m_filteredExtensionProcessors;
};
