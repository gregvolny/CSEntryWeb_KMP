...
static readstat_error_t dta_emit_xmlish_header(readstat_writer_t *writer, dta_ctx_t *ctx) {
    ...
    if (writer->version >= 118) {
        ...
        if (error != READSTAT_OK)
            goto cleanup;
#endif
        error = dta_write_tag(writer, ctx, "<N>");
        if (error != READSTAT_OK)
            goto cleanup;

        if( writer->row_count_position_recorder != NULL )
            writer->row_count_position_recorder(writer->bytes_written, sizeof(nobs), 1, writer->object_holding_writer);

        error = readstat_write_bytes(writer, &nobs, sizeof(nobs));
        if (error != READSTAT_OK)
            goto cleanup;
...
static readstat_error_t dta_emit_map(readstat_writer_t *writer, dta_ctx_t *ctx) {
    ...
    // CSPRO_READSTAT_MODIFICATIONS
    int64_t nobs_temp = ctx->nobs;

    if( writer->dta_emit_map_position_recorder != NULL )
        ctx->nobs = 0;
    ...
    if( writer->dta_emit_map_position_recorder != NULL )
    {
        size_t emit_map_position = writer->bytes_written +
                                   sizeof("<map>") - 1 + // - 1 because no null character is written
                                   sizeof(uint64_t) * 10;

        writer->dta_emit_map_position_recorder(emit_map_position, ctx->record_len,
            map[10], map[11], map[12], map[13], writer->object_holding_writer);
    }
