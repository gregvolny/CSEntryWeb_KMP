...
#pragma warning(disable:4267) // CSPRO_READSTAT_MODIFICATIONS
...
static readstat_error_t sav_emit_header(readstat_writer_t *writer) {
    ...
    memcpy(header.prod_name,
           // CSPRO_READSTAT_MODIFICATIONS
           // "@(#) SPSS DATA FILE - " READSTAT_PRODUCT_URL, 
           // sizeof("@(#) SPSS DATA FILE - " READSTAT_PRODUCT_URL)-1);
           "@(#) SPSS DATA FILE - " READSTAT_CSPRO_NAME, 
           sizeof("@(#) SPSS DATA FILE - " READSTAT_CSPRO_NAME)-1);
    ...
    // CSPRO_READSTAT_MODIFICATIONS
    if( writer->row_count_position_recorder != NULL ) 
    {
        size_t ncases_offset = sizeof(header.rec_type) +
                               sizeof(header.prod_name) +
                               sizeof(header.layout_code) +
                               sizeof(header.nominal_case_size) +
                               sizeof(header.compression) +
                               sizeof(header.weight_index);

        writer->row_count_position_recorder(writer->bytes_written + ncases_offset,
            sizeof(header.ncases), 0, writer->object_holding_writer);
    }
...
static readstat_error_t sav_emit_number_of_cases_record(readstat_writer_t *writer) {
    ...
    // CSPRO_READSTAT_MODIFICATIONS
    if( writer->row_count_position_recorder != NULL )
        writer->row_count_position_recorder(writer->bytes_written, sizeof(ncases), 1, writer->object_holding_writer);
...
/* CSPRO_READSTAT_MODIFICATIONS static */readstat_error_t sav_validate_name_unreserved(const char *name) {
