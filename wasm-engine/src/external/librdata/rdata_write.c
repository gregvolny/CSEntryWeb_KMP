...
void rdata_writer_reset_columns(rdata_writer_t *writer) {
    // CSPRO_LIBRDATA_MODIFICATIONS this function has been added
    // to support adding a different number of columns to each table
    int i, j;
    ...
    writer->columns_count=0;
}

void rdata_writer_free(rdata_writer_t *writer) {
    ck_hash_table_free(writer->atom_table);
    // CSPRO_LIBRDATA_MODIFICATIONS freeing of columns moved to the function above
    rdata_writer_reset_columns(writer);
  /*int i, j;
    for (i=0; i<writer->columns_count; i++) {
        rdata_column_t *column = writer->columns[i];
        for (j=0; j<column->factor_count; j++) {
