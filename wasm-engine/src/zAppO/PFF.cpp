...
void PFF::SetAppFName(const TCHAR* filename)
{
    ...
    printf("[PFF] SetAppFName called with: '%ls'\n", (const wchar_t*)filename);
    printf("[PFF] PIF filename (for working folder): '%ls'\n", (const wchar_t*)m_sPifFileName);
#endif
    SetFilename(m_sAppFName, filename);
#ifdef __EMSCRIPTEN__
    printf("[PFF] After SetFilename, m_sAppFName = '%ls'\n", (const wchar_t*)m_sAppFName);
#endif
...
{
#ifdef __EMSCRIPTEN__
    printf("[PFF] LoadPifFile called for: %ls\n", (const wchar_t*)m_sPifFileName);
#endif
    ...
    {
#ifdef __EMSCRIPTEN__
        printf("[PFF] Failed to open PIF file: %ls\n", (const wchar_t*)m_sPifFileName);
#endif
        return false;
    }

#ifdef __EMSCRIPTEN__
    printf("[PFF] PIF file opened successfully\n");
#endif
    ...
    while( nPifFile.GetLine(command, argument, false) == SF_OK )
    {
        ...
        if( read_run_info )
        {
            ...
            {
#ifdef __EMSCRIPTEN__
                printf("[PFF] Command: '%ls' = '%ls'\n", (const wchar_t*)command, (const wchar_t*)argument);
#endif
